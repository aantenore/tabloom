...
Loading one local model runtime in every sibling page can multiply scarce browser memory. A shared worker can centralize work, but lifecycle support and runtime compatibility vary, and a dedicated worker still belongs to one page. The alpha needs an observable, replaceable approach that can be tested in ordinary pages.

## Decision

Use an exclusive Web Lock to elect one page in the current storage bucket. Assign a monotonic epoch from same-origin storage while the lock is held. Use BroadcastChannel for protocol envelopes. Put both behind ports so a SharedWorker transport, service worker bridge, or test coordinator can replace them later.
...
- SharedWorker: valuable future adapter, but not the only core topology.
